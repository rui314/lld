...
public:
  ...
  NewArchiveIterator create(const Export &E) {
    ...
    bool Decorated = E.ExtLibName.startswith("?") ||
                     E.ExtLibName.startswith("@");
    ...
    sprintf(Hdr->Size, "%-10d", sizeof(coff_import_header) + ImpSize);
...
void writeImportLibrary() {
  ...
  std::pair<StringRef, std::error_code> Result = writeArchive(
      Path, Members, /*WriteSymtab*/true, object::Archive::K_GNU,
